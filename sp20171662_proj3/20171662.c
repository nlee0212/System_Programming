...
int main(){
	...
	//initializing Object Code Linked List
	...
	while(fgets(opcode_input,80,fp)!=NULL){
		...
		strcpy(op_mem->format,mod_opinput[2]);
	...
	while(1){
		...
		//for checking ','
		if(size>=3 && strcmp("loader",modified_input[0])!=0){
			for(i=1;i<size-1;i++){
		...
		//if type filename
			...
			if(type(modified_input[1])) insertNode(head,save_input);
			continue;
		}

		//if assemble filename
		else if(!strcmp("assemble",modified_input[0]) && size == 2){
			if(assemble(modified_input[1])) insertNode(head,save_input);
			continue;
		}
		...
		//if loader [object filename1] [object filename2] [...]
			...
			for(i=1;i<size;i++){
				//execute pass 2
				if(!linkload_p2(modified_input[i],i-1,size-1)) break;
			}
...
char* right_input_form(int size, char** m_input){
	...
	for(i=0;i<size;i++){
		...
		if(i>=1 && size>i+1 && !strcmp(m_input[0],"loader")) strcat(ret_input," ");
		else if(i>=1 && size>i+1){
			strcat(ret_input,", ");
		}
